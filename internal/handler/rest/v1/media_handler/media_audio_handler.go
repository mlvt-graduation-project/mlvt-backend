...
func (h *MediaController) GenerateUploadURL(c *gin.Context) {
	...
	url, err := h.mediaService.GeneratePresignedUploadURL(folder, fileName, fileType)
	if err != nil {
...
func (h *MediaController) GenerateDownloadURL(c *gin.Context) {
	...
	downloadURL, err := h.mediaService.GeneratePresignedDownloadURL(audioID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
// @Failure 500 {object} response.ErrorResponse "error"
// @Router /audios [post]
func (h *MediaController) AddAudio(c *gin.Context) {
...
func (h *MediaController) GetAudio(c *gin.Context) {
	audioIDStr := c.Param("audio_id")
	audioID, err := strconv.ParseUint(audioIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid audio ID"})
	...
	audio, downloadURL, err := h.mediaService.GetAudioByID(audioID)
	if err != nil {
...
func (h *MediaController) GetAudioByUser(c *gin.Context) {
	// Parse audio ID from the URL path
	audioIDStr := c.Param("audio_id")
	audioID, err := strconv.ParseUint(audioIDStr, 10, 64)
	if err != nil {
	...
	audio, downloadURL, err := h.mediaService.GetAudioByIDAndUserID(audioID, userID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) ListAudiosByUserID(c *gin.Context) {
	userIDStr := c.Param("user_id")
	userID, err := strconv.ParseUint(userIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid user ID"})
	...
	transcriptions, err := h.mediaService.ListAudiosByUserID(userID)
	if err != nil {
...
func (h *MediaController) GetAudioByVideoID(c *gin.Context) {
	// Parse audio ID from the URL path
	audioIDStr := c.Param("audio_id")
	audioID, err := strconv.ParseUint(audioIDStr, 10, 64)
	if err != nil {
	...
	// Call the service to get the audio and the presigned download URL
	audio, downloadURL, err := h.mediaService.GetAudioByVideoID(videoID, audioID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) ListAudiosByVideoID(c *gin.Context) {
	videoIDStr := c.Param("video_id")
	videoID, err := strconv.ParseUint(videoIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid video ID"})
	...
	audios, err := h.mediaService.ListAudiosByVideoID(videoID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: "internal server error"})
		return
	}
