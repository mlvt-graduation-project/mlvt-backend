...
func (h *MediaController) GetVideoStatus(c *gin.Context) {
	...
	status, err := h.mediaService.GetVideoStatus(videoID)
	if err != nil {
...
func (vc *MediaController) UpdateVideoStatus(c *gin.Context) {
	videoIDStr := c.Param("video_id")
	videoID, err := strconv.ParseUint(videoIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid video ID"})
	...
	err = vc.mediaService.UpdateVideoStatus(videoID, req.Status)
	if err != nil {
...
// @Failure 500 {object} response.ErrorResponse
// @Router /videos [post]
func (h *MediaController) AddVideo(c *gin.Context) {
...
func (h *MediaController) GenerateUploadURLForVideo(c *gin.Context) {
	folder := env.EnvConfig.VideosFolder
	fileName := c.Query("file_name")
	fileType := c.Query("file_type")

	url, err := h.mediaService.GeneratePresignedUploadURLForVideo(folder, fileName, fileType)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) GenerateUploadURLForImage(c *gin.Context) {
	folder := env.EnvConfig.VideoFramesFolder
	fileName := c.Query("file_name")
	fileType := c.Query("file_type")

	url, err := h.mediaService.GeneratePresignedUploadURLForImage(folder, fileName, fileType)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) GenerateDownloadURLForVideo(c *gin.Context) {
	videoIDStr := c.Param("video_id")
	videoID, err := strconv.ParseUint(videoIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid video ID"})
	...
	downloadURL, err := h.mediaService.GeneratePresignedDownloadURLForVideo(videoID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) GenerateDownloadURLForImage(c *gin.Context) {
	videoIDStr := c.Param("video_id")
	videoID, err := strconv.ParseUint(videoIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid video ID"})
	...
	downloadURL, err := h.mediaService.GeneratePresignedDownloadURLForImage(videoID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{Error: err.Error()})
		return
	}
...
func (h *MediaController) GetVideoByID(c *gin.Context) {
	videoIDStr := c.Param("video_id")
	videoID, err := strconv.ParseUint(videoIDStr, 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.ErrorResponse{Error: "invalid video ID"})
	...
	video, videoURL, imageURL, err := h.mediaService.GetVideoByID(videoID)
	if err != nil {
...
func (h *MediaController) ListVideosByUserID(c *gin.Context) {
	...
	videosWithURLs, err := h.mediaService.ListVideosByUserID(userID)
	if err != nil {
